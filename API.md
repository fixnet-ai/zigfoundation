...
pub const Registry = struct {
    ...
    pub fn dial(self: *Registry, comptime buf_size: usize, name: []const u8) !MemStream;  // 同步
...
| 方法 | 签名 | 描述 |
|------|------|------|
| `createPair` | `fn (comptime buf_size: usize, loop_a, loop_b, allocator) !PairHandle` | 创建一对已连接的 MemStream |
...
// 异步 accept
var accepted: ?memconn.MemStream = null;
var ac: xev.Completion = .{};
listener.accept(&loop, &ac, ?memconn.MemStream, &accepted, (struct {
    fn cb(ud: ?*?memconn.MemStream, l: *xev.Loop, c: *xev.Completion, conn: ?memconn.MemStream) xev.CallbackAction {
        _ = l; _ = c;
...
| `accept` | `fn (?*Userdata, *xev.Loop, *xev.Completion, ?MemStream) xev.CallbackAction` | `null` = listener 已关闭 |
...
| 0.1.2 | 2026-07-19 | Phase 8: memconn.zig 内存网络连接模块（MemPipe/MemStream/PairHandle/MemListener/Registry），37 tests，233 total tests 全绿，zero memory leaks |

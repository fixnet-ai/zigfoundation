...
- [x] 创建 .claude/skills/ 软链接（zig 目录、utm-vm → ../../../utm-monitor/utm-vm）
...
### Phase 10: 文档矛盾修复 + log.zig 平台增强 + README (2026-07-20)

**6 个文档/注释矛盾点修复**:

1. [x] `memconn.zig:499` — `PairHandle.destroy()` 注释: "可安全多次调用" → "不可多次调用"
2. [x] `foundation.zig:32/37/43` — Phase 依赖标注匹配代码现状（Phase 3: std+zli, Phase 4: std+libyaml, Phase 5: std only）
3. [x] `API.md:59,71` — buffer `defaultConfig()` 文档: 初始 256 块→0 块（按需扩展）
4. [x] `store.zig` — 原子写入 fsync: 保持现状，信任 OS
5. [x] `log.zig` — Android: `__android_log_write`(logcat) / iOS: `syslog` / macOS+桌面: stderr
6. [x] `README.md` — 创建简洁 GitHub README（模块表+构建+平台+许可）

- [x] 回归: `zig build test` 219/219 ✅ + `zig fmt --check` ✅
- **Status:** complete

### Phase 10a: relay.zig 双向数据中继 + fdconn.zig 流适配器 (2026-07-20)

- [x] `relay.zig` — 通用异步双向数据中继模块：FdStream 适配器、RelayCtx Completion 链、4 tests
- [x] memconn.close() 改为同步完成（修复与 pending read 在 self_read_async 上的 kqueue 冲突）
...
| 平台 | 构建 | 测试 | 备注 |
|------|------|------|------|
| macOS aarch64 (host) | ✅ | 239/239 ✅ + CLI 15/15 ✅ | 本地原生 |
| Linux aarch64-musl | ✅ | 239/239 ✅ | UTM VM (`utmm --upload` + `--exec linuxvm`) |
| macOS aarch64 (VM) | ✅ | 239/239 ✅ | UTM VM (`utmm --exec macvm`) |
| iOS aarch64-simulator | ✅ | 13/13 ✅ | `simctl spawn booted` 真机测试 |
| Android aarch64 | ✅ | 13/13 ✅ (exit 0) | `adb shell` 模拟器 + logcat 确认 |
| Windows aarch64-gnu | ✅ PE32+ | `*` | 构建上传成功，utmm Windows Guest exec 输出回传限制 |

> `*` Windows VM: 同代码在其他 5 个平台 239/239 全绿，交叉编译 PE32+ 成功，utmm `--exec windowsvm` 连接正常但命令输出未回传。

- [x] macOS host — 239/239 ✅ + CLI 15/15 ✅
- [x] Linux aarch64-musl UTM VM — 239/239 ✅
- [x] macOS aarch64 UTM VM — 239/239 ✅
- [x] iOS aarch64-simulator — 13/13 ✅
- [x] Android aarch64 模拟器 — 13/13 ✅
- [x] Windows aarch64-gnu — PE32+ 构建 ✅（VM 输出捕获限制）
- **Status:** complete

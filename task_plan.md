...
## Goal
从 zigproxy/zproxy/zigtun 提取公共组件，实现 14 个工业级基础模块（buffer/ring/endian/platform/net/strings/cli/log/yaml/store/event/queue/egress/memconn），100% 单元测试覆盖，五平台支持。

## Current Phase
Phase 11: 五平台 VM 测试 + 文档同步 — 进行中（Windows/iOS VM 测试待 utmm 更新后继续）
...
| 平台 | 构建 | VM 测试 | 备注 |
|------|------|---------|------|
| macOS aarch64 (host) | ✅ | 219/219 ✅ + CLI 13/13 ✅ | 本地 |
| Linux aarch64-musl | ✅ | 219/219 ✅ | UTM VM (`utmm --upload` + `--exec`) |
| iOS aarch64-simulator | ✅ | — | 二进制就绪，需 simctl (macvm) |
| Android aarch64 | ✅ | 13/13 ✅ | 模拟器 (adb shell, Phase 6e) |
| Windows aarch64-gnu | ✅ | — | 二进制就绪，utmm 更新中 |

- [x] macOS host — 219/219 ✅ + CLI 13/13 ✅
- [x] Linux aarch64-musl UTM VM — 219/219 ✅
- [x] Android aarch64 模拟器 — 13/13 ✅（Phase 6e）
- [ ] iOS simctl 真机测试 — 二进制就绪，待 utmm 更新后通过 macvm 执行
- [ ] Windows aarch64-gnu 真机测试 — PE32+ 构建成功，待 utmm 更新后通过 windowsvm 执行
- **Status:** in_progress（阻塞于 utmm 更新）
